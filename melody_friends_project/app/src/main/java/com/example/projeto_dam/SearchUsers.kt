...
class SearchUsers : AppCompatActivity() {
            ...
            if (documents != null) {
                ...
                for (document in documents) {
                    ...
                    if (document.get("instruments") != null) {
                        ...
                        strInstruments = arrToString(arrInstruments)

                    }


                    createLayout(userName!!, strInstruments)

                    if (userImage != ""){

                        Picasso.with(this).load(userImage).fit().centerInside().into(profiles!!.get(c))

                    }
                    setListenerToLayouts(userId!!,c)
    ...
    private fun setListenerToLayouts(otherUser: String, index: Int) {
        layUsers!![index].setOnClickListener {
            val intent = Intent(this, UserProfile::class.java)
